		...
		<div className=" bg-white w-full h-full overflow-hidden ">
			<div className="flex flex-col h-full w-full">
				<div className="flex w-full items-center space-x-2 pb-2">
					<Input type="email" placeholder="Query..." onChange={(e) => setQuery(e.target.value)} />
					<Button type="submit" onClick={() => handleSearch()}>Search</Button>
				</div>
				<div className="flex flex-row h-full items-start justify-start">
					<div className="max-w-2/3 border-r h-full px-2">
						{collate}
					</div>
            ...
            {references.map((ref, index) => (
              ...
              <CardHeader>
                ...
                <CardDescription className="text-xs">{ref.chunk_id}</CardDescription>
              </CardHeader>
