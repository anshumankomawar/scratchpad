        ...
        <div className="flex flex-col w-full">
            <div className='flex items-center space-x-8 w-full px-12'>
                <Input
                    ...
                    disabled={false}/>
                <Search className="h-4 w-4 hover:cursor-pointer" onClick = {() => handleSearch()} />
                <Bookmark className={`h-4 w-4 hover:cursor-pointer${isBookMarked ? " fill-current text-black" : ""}`} onClick={() => handleBookmarkClick()}/>
            </div>
            <div className="text-start px-12 pt-8 rounded-md text-sm w-full text-slate-900 pb-4">{searchText}</div>
        </div>
