...
export default function BottomPanel({theme, setTheme, editor, toggleLeftPanel, toggleBottomPanel, handleLogout}) {
  ...
  return (
    <Dialog open={openDialog}>
      <div className="z-100 bottom-0 left-0 fixed h-min flex flex-row w-full font-virgil items-center px-5 py-4 bg-white dark:bg-stone-900">
        <Popover>
          <PopoverContent className='p-1 ml-2 mb-2 flex flex-col overflow-y-scroll space-y-1 w-32 font-virgil text-xs text-start dark:bg-stone-900'>
            ...
            <Button variant="ghost" size="menu" className="text-xs justify-start p-2" onClick={() => setOpenDialog(true)}>Settings</Button>
      ...
      <DialogContent className="bg-white h-5/6 w-5/6 font-virgil p-8" onPointerDownOutside={() => setOpenDialog(false)}>
        <div className='flex flex-col'>
          <div className='text-bold text-lg pb-1'>Settings</div>
          <div className='text-sm text-gray-400'>Mange your account preferences.</div>
